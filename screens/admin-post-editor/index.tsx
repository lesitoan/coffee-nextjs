      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
        <div className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-center">
          ...
          <div className="flex gap-2">
            {post ? (
              <button type="button" onClick={deletePost} disabled={saving} className="rounded-xl border border-red-200 px-4 py-2 text-sm font-bold text-red-700 transition hover:bg-red-50 disabled:opacity-60">
                Delete
              </button>
            ) : null}
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <label className="md:col-span-2">
            ...
            <input name="title" defaultValue={initial.title} required className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-bold text-stone-700">Slug</span>
            <input name="slug" value={slug} onChange={(event) => setSlug(event.target.value)} placeholder="egg-coffee-secrets" className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-bold text-stone-700">Status</span>
            <select name="status" defaultValue={initial.status} className="w-full rounded-xl border border-stone-300 px-4 py-3">
          ...
          <label>
            ...
            <select name="category" defaultValue={initial.category} required className="w-full rounded-xl border border-stone-300 px-4 py-3">
          ...
          <label>
            ...
            <input name="publishedAt" type="date" defaultValue={initial.publishedAt} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-bold text-stone-700">Reading Time</span>
            <input name="readingTime" defaultValue={initial.readingTime} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-bold text-stone-700">Tags, comma separated</span>
            <input name="tags" defaultValue={initial.tags?.join(", ")} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          ...
          <label className="md:col-span-2">
            ...
            <textarea name="excerpt" defaultValue={initial.excerpt} rows={3} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <label>
            ...
            <input name="seoTitle" defaultValue={initial.seoTitle} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <label>
            <span className="mb-2 block text-sm font-bold text-stone-700">Cover Alt</span>
            <input name="coverAlt" defaultValue={initial.coverAlt} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <label className="md:col-span-2">
            <span className="mb-2 block text-sm font-bold text-stone-700">SEO Description</span>
            <textarea name="seoDescription" defaultValue={initial.seoDescription} rows={3} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
          </label>
          <div className="md:col-span-2">
            <span className="mb-2 block text-sm font-bold text-stone-700">Cover Image</span>
            <div className="flex flex-col gap-3 md:flex-row">
              <input value={coverImage} onChange={(event) => setCoverImage(event.target.value)} className="w-full rounded-xl border border-stone-300 px-4 py-3" />
              <input type="file" accept="image/*" onChange={(event) => event.target.files?.[0] && uploadCover(event.target.files[0]).catch((uploadError) => setError(uploadError.message))} className="rounded-xl border border-stone-300 px-4 py-3 text-sm" />
            </div>
          </div>
        </div>
      </div>
