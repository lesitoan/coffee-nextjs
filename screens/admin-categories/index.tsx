  ...
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-center">
        ...
        <Link href="/admin/categories/new" className="rounded-xl bg-coffee-700 px-5 py-3 text-sm font-bold text-white transition hover:bg-coffee-800">
          Create Category
        </Link>
      </div>
        ...
        {categories.map((category) => (
          <div key={category.id} className="flex flex-col justify-between gap-3 py-4 md:flex-row md:items-center">
            ...
            <div className="flex items-center gap-3">
              <span className="rounded-full bg-stone-100 px-3 py-1 text-xs font-bold text-stone-700">{category.status || "active"}</span>
